...
- **Fullscreen is a specificity trap.** `body.touch .game-wrap` (0,2,1) outranks `.game-wrap:fullscreen` (0,2,0), so the touch layout's margin and chrome-sized width leak into fullscreen and pin the arena to the top of the screen. Any new `body.touch .game-wrap` rule needs a matching `body.touch .game-wrap:fullscreen` / `:-webkit-full-screen` pair, written as separate rules — one unknown selector invalidates a whole comma group. Note there is no fullscreen at all on iPhone Safari: `requestFullscreen` is missing on the element, so `toggleFullscreen()` silently does nothing there.
...
| `PASSIVES`, `GLOBALS` | tree passives (`cost`/`max`, `branch`, `req`/`reqMax`) and the in-run system upgrades they unlock — `GLOBALS` holds the level-up card text, pause-menu line and tree tooltip in one place so the three cannot disagree |
