...
- `sectorsOpen` (persisted as `shapeshift_sectors`) gates the picker, the tree **and the hangar**: any node carrying `sector: 2`/`sector: 3` is invisible via `nodeVisible` until that sector is open, and any `characters` entry carrying a `sector` is *sealed* via `pilotSealed` — shown on its card but unbuyable at any price, the way the sector reel shows what is next rather than hiding it. The seal is on acquiring a hull, not on keeping one: an airframe already in the hangar stays flyable. Clearing `FINAL_ROOM` on hard opens the next one, in `runCleared`. `showTree()` draws the whole V2 half (ARMAMENTS V2 / SYSTEMS V2 / AMPLIFIERS V2) only when `sectorOpen(2)`; otherwise it prints one sealed panel in its place.
...
| `characters` | pilot roster: cost, hp & speed multipliers, exclusive `weapon`, perks, hull silhouette, and an optional `sector` sealing it in the hangar until that sector is open. A `perks` entry may be a **function** when the wording depends on the control scheme — PARAGON's shockwave line reads `ctrlWave()` — the same escape hatch `RELICS` has for `desc`/`line` |
